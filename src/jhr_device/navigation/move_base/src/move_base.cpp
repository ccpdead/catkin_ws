...
namespace move_base
{
  ...
  bool MoveBase::executeCycle(geometry_msgs::PoseStamped &goal)
  {
    ...
    if (new_global_plan_)
    {
      ...
      if (!tc_->setPlan(*controller_plan_))
      {
        ...
        ROS_ERROR("----Duration 2 second----");
        ros::Duration(2).sleep();//在这里休眠2秒
    ...
    case CONTROLLING:
...

      {
        ...
        if (tc_->computeVelocityCommands(cmd_vel))//DWA局部路径规划成功
        {
        ...
        else//￥ --------------------DWA路径规划失败
        {
          ...
          if (ros::Time::now() > attempt_end)//如果局部规划时间超时
          {
            // we'll move into our obstacle clearing mode
            publishZeroVelocity();
            state_ = CLEARING;//movebase状态位标志位清除状态
            recovery_trigger_ = CONTROLLING_R;//恢复标志设置位控制状态中。
          }
          else
          {
            ...
            last_valid_plan_ = ros::Time::now();
            planning_retries_ = 0;
            state_ = PLANNING;//配置系统标志为规划中，从新进入全局规划中
            ROS_ERROR("DWA compute failed,Delay 5second!, replaning.......");
    ...
    case CLEARING: //@ 进入恢复模式
      ROS_DEBUG_NAMED("move_base", "In clearing/recovery state");
      //!----------------------------------Executing behavior1,2,3,4--------------------------
      if (recovery_behavior_enabled_ && recovery_index_ < recovery_behaviors_.size())
      {
        ...
        msg.current_recovery_number = recovery_index_;                          //当前恢复模式索引0，1，2，3
        msg.total_number_of_recoveries = recovery_behaviors_.size();            //恢复模式数组大小--4
        msg.recovery_behavior_name = recovery_behavior_names_[recovery_index_]; //保守复位，轮换复位，主动复位，轮换复位
        ...
        if (recovery_index_ == 1)
        {
          ...
          if (MoveBase::clearCostmapsService(req, resp))
          {
            ROS_INFO("----clearCostmapsService allowed-----");
          }
        }
  ...
  {
    ROS_WARN("Loding default recovery behaviors-------------------------------");
    recovery_behaviors_.clear(); //删除容器中所有元素
    try
    {
